...
│   ├── opsFlightSchedule.ts   # 380-movement peak-day fixture + weekly variation
...
```
Flights land at HKT Airport (380-movement peak-day fixture, real airlines/origins)
  → Passengers clear customs (20-45 min model)
  → 3–7% want the bus by origin heuristic (~5% weighted fleet-wide)
...
The operations engine uses a 380-movement fixture: 190 arrivals + 190 departures, 29 airlines and 36 cities, with deterministic day-of-week cancellations, load variation and charters. Capture varies by origin from 3–7% and averages roughly 5% fleet-wide. `flightData.ts` remains the smaller tourist-app window; it is not the v2 operations source of truth.
...
Don't hardcode "1,200 riders today." Instead: scheduled flight pax × origin-based capture heuristic × customs clearance model × bus capacity × timetable = derived riders. Every number must be traceable.
