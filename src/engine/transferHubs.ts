import type { MetricProvenance, OperationalRouteId, TransferHub } from "@shared/types";
...
const HUBS: HubConfig[] = [
  {
    ...
    rationale: text("Modeled feeder hub linking Old Town services to the Rassada ferry bank.", "จุดเชื่อมต่อแบบจำลองที่เชื่อมบริการเมืองเก่ากับท่าเรือรัษฎา")
  },
  {
    id: "chalong",
    ...
    walkMinutes: 15,
    transferBufferMinutes: 20,
    rationale: text("Modeled feeder hub linking the south corridor to Chalong Pier departures.", "จุดเชื่อมต่อแบบจำลองที่เชื่อมคอร์ริดอร์ฝั่งใต้กับเที่ยวเรือท่าเรือฉลอง")
  },
  {
    id: "bang-rong",
    ...
    rationale: text("Modeled feeder hub linking the airport corridor to Bang Rong pier services.", "จุดเชื่อมต่อแบบจำลองที่เชื่อมคอร์ริดอร์สนามบินกับเที่ยวเรือบางโรง")
  }
];
...
function buildFeederConnections(hub: HubConfig, currentMinutes: number) {
  return getBusScheduledServices()
    .filter((service) => hub.feederRouteIds.includes(service.routeId) && service.inferredHubTravelMinutes !== null)
    .map((service) => {
      ...
      return {
        ...
        minutesUntil: nextArrivalMinutes === null ? null : Math.max(0, nextArrivalMinutes - currentMinutes),
...
function buildFerryConnections(hub: HubConfig, currentMinutes: number) {
  return getFerryScheduledServices()
    .filter((service) => hub.ferryRouteIds.includes(service.routeId))
    .map((service) => {
...
function resolveHubStatus(hub: HubConfig, ferryConnections: ReturnType<typeof buildFerryConnections>) {
  const nextFerry = ferryConnections[0];
  if (!nextFerry || nextFerry.minutesUntil === null) {
    return { status: "inactive" as const, activeWindowLabel: null, nextWindowStartLabel: null };
  }

  const readyThreshold = hub.walkMinutes + hub.transferBufferMinutes;

  if (nextFerry.minutesUntil <= readyThreshold) {
    return { status: "ready" as const, activeWindowLabel: `${nextFerry.nextDepartureLabel} ferry window`, nextWindowStartLabel: nextFerry.nextDepartureLabel };
  }

  if (nextFerry.minutesUntil <= readyThreshold + 30) {
    return { status: "watch" as const, activeWindowLabel: null, nextWindowStartLabel: nextFerry.nextDepartureLabel };
  }

  return { status: "inactive" as const, activeWindowLabel: null, nextWindowStartLabel: nextFerry.nextDepartureLabel };
}
...
export function getTransferHubs(now = new Date(), provenance: MetricProvenance = "estimated"): TransferHub[] {
  ...
  return HUBS.map((hub) => {
    ...
    const feederConnections = buildFeederConnections(hub, currentMinutes);
