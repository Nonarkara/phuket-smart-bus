...
import { startPhuketGpsProducer, stopPhuketGpsProducer, getPhuketProducerState } from "./engine/phuketGpsProducer";
...
export default function DashboardV2() {
  ...
  useEffect(() => {
    // Start the always-on Phuket upstream GPS producer the moment the
    // operator wall mounts. It runs in the background, polls every 30 s,
    // and feeds `ingestBatchGps` which both lights up the live telemetry
    // map AND records into the fleet efficiency ledger.
    const stop = startPhuketGpsProducer(30_000);
    const tick = () => {
  ...
  return (
    <div className={`v2 v2--${viewMode}`} style={{ zoom: opsScale }}>
      ...
      <header className="v2-header">
        ...
        <button
          type="button"
          className={`v2-header__telemetry-pill ${isGpsActive ? "is-live" : producerState.lastError ? "is-error" : "is-sim"}`}
          onClick={() => setIsTelemetryModalOpen(true)}
          title={
            producerState.lastError
              ? `Live feed error: ${producerState.lastError} — click to inspect`
              : "Click to inspect Live GPS Telemetry Console or test live hardware ingestion"
          }
        >
          <span className="v2-telemetry-dot" />
          <span>
            {producerState.running && producerState.lastSuccessAt === null
              ? "FETCHING GPS…"
              : isGpsActive
                ? `LIVE GPS · ${liveGpsCount} BUSES`
                : producerState.lastError
                  ? "GPS FEED ERROR"
                  : "TIMETABLE SIM"}
          </span>
